...
(function () {
  ...
  const quizStorageKey   = 'codebase-course-quiz-level';
  let quizLevel          = 'medium';

  function resetQuizState(root) {
    if (!root) return;
    $$('.quiz-option', root).forEach(option => {
      option.classList.remove('selected', 'correct', 'incorrect');
      option.disabled = false;
    });
    $$('.quiz-feedback', root).forEach(feedback => {
  ...
  try {
    ...
    if (savedLevel) quizLevel = savedLevel;
  } catch (err) {}
  ...
  window.checkQuiz = function (containerId) {
    ...
    $$('.quiz-question-block', scope).forEach(q => {
